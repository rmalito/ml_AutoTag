...
def do_autoTag(param_inputfile):
    path_var = "python pdf2txt.py -o autoTag.html " + param_inputfile
    os.system(path_var)
...
def do_autoTag_img(param_inputfile):
    # AKIAJQQTVH4CVEI7VWKA
    # Wfonu2NxaLvgNvbLXGg6VskcPrjOjJKxy3GExSyf
    BUCKET = "wowfamrekog"
    KEY = param_inputfile
    print("\n")
...
def detect_labels(bucket, key, max_labels=15, min_confidence=10, region="us-east-1"):
...
# print("\n")
inputfile = input("AutoTag(pdf/jpg/png): ")
# inputfile = "C:/Users/rmalito/AnacondaProjects/LDA_Testing/test_docs/"

# directory = os.fsencode(inputfile)
...
#         count += 1
#         print(f'{os.path.join(inputfile, filename)} count: {count}')
#         continue
#     else:
#         print(f'{os.path.join(inputfile, filename)} is not .pdf...skipping')
#         continue

if inputfile.endswith(".pdf"): 
    # count += 1
    # print(f'{os.path.join(inputfile, filename)} count: {count}')
    do_autoTag(inputfile)
elif inputfile.endswith(".jpg"):
    do_autoTag_img(inputfile)
elif inputfile.endswith(".png"):
    do_autoTag_img(inputfile)
else:
    print(f'{inputfile} is not valid type...')
